 ...
 */
#define MBM_TIME_DELTA_MIN	(100 * 1)
...
static u64 rmid_read_mbm(unsigned int rmid, enum mbm_evt_type evt_type)
{
	...
	u64 diff, tmp32 = rmid, val1=val,val2= (prevavg * (cqm_l3_scale + 500)/1000);
	printk(KERN_WARNING "msr %d lastmsr %d  val %u L/T %d BW  %u MB/sec    diff_time %u cma %u index %d  cpu  %u pkg_id %d rmid %d vrmid %d window_size %d\n",currentmsr,bytes,val,evt_type,val2, diff_time,prevavg,index,smp_processor_id(),topology_physical_package_id(smp_processor_id()),tmp32,vrmid,mbm_window_size);
			...
			*/
			...
			if (index    && (index < mbm_window_size)) {
				...
				averagebw = (bwsum + currentbw) /
					     mbm_window_size;
				printk(KERN_WARNING "avg2 %d  ",averagebw);
			}
...
static void __intel_mbm_event_count(void *info)
{
	...
	if (val & (RMID_VAL_ERROR | RMID_VAL_UNAVAIL)) {
		printk(KERN_WARNING " RMID ERROR ");
		return;
	}
	printk(KERN_WARNING "rmid cpu %d val %u",smp_processor_id(),val);
	atomic64_add(val, &rr->value);
}
...
static int intel_cqm_event_init(struct perf_event *event)
{
	...
	if (group) {
		...
		if (!__rmid_valid(event->hw.cqm_rmid))
			rotate = true;
	printk(KERN_WARNING "not group rotate %d  ",rotate);
	}
