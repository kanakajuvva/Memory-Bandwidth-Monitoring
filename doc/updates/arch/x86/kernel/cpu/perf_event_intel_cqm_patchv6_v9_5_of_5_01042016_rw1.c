 ...
 */
#if 1
//static DEFINE_PER_CPU(struct sample *, mbm_total);
//static DEFINE_PER_CPU(struct sample *, mbm_local);
#endif
 ...
 */
#define rmid_2_index(rmid, cpu)  (pkg_id * cpu * cqm_max_rmid + rmid)
...
static u64 rmid_read_mbm(unsigned int rmid, enum mbm_evt_type evt_type)
{
	...
	ktime_t cur_time;
	u32 eventid, index, cpu = smp_processor_id() ;
	struct sample *mbm_current, *tmp;
	u32 vrmid = rmid_2_index(rmid, cpu);

	//cur_time = ktime_get();
	if (evt_type & QOS_MBM_LOCAL_EVENT_MASK) {
		//tmp =  __this_cpu_read(mbm_local);
		//rcu_assign_pointer(per_cpu(mbm_local, cpu), tmp);
		mbm_current = &mbm_local[vrmid];
		eventid     =  QOS_MBM_LOCAL_EVENT_ID;
	} else {
		//tmp =  __this_cpu_read(mbm_total);
		//rcu_assign_pointer(per_cpu(mbm_total, cpu), tmp);
		mbm_current = &mbm_total[vrmid];
		eventid     = QOS_MBM_TOTAL_EVENT_ID;
	}

	write_lock(&mbm_current->rwlock);
	cur_time = ktime_get();
	...
	printk(KERN_WARNING "msr %d lastmsr %d  val %u L/T %d BW  %u MB/sec    diff_time %u cma %u index %d  cpu  %u pkg_id %d      rmid %d vrmid %d window_size %d\n",currentmsr,bytes,val, evt_type,val2, diff_time,prevavg,index,smp_processor_id(),topology_physical_package_id(smp_processor_id()),rmid,vrmid,mbm_window_size);
	...
	if (diff_time > MBM_TIME_DELTA_MIN) {
		...
		if ((diff_time <= (MBM_TIME_DELTA_EXP + MBM_TIME_DELTA_MIN))  ||
			...
			*/
			...
			averagebw = currentbw;
			printk(KERN_WARNING "cal1 %d  ",currentbw);
			if (index    && (index < mbm_window_size)) {
				...
				averagebw = (bwsum + currentbw) /
					     mbm_window_size;
				printk(KERN_WARNING "avg2 %d sum %d  ",averagebw,bwsum);
	...
	write_unlock(&mbm_current->rwlock);
...
static int  intel_mbm_init(void)
{
	...
	mbm_socket_max++;
#if 1
	array_size = (cqm_max_rmid + 1) * mbm_socket_max * NR_CPUS;
