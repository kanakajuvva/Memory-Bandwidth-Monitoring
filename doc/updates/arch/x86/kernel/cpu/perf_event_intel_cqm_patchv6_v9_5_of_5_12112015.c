...
#define mbmfifo_last_get(stat) ((stat->fifoin == 0) ?
		(stat->runavg) : (stat->mbmfifo[stat->fifoin-1]))
...
static u64 rmid_read_mbm(unsigned int rmid, enum mbm_evt_type evt_type)
{
	...
	currentbw = mbmfifo_last_get(mbm_current); //mbm_current->mbmfifo[mbm_current->fifoin];
