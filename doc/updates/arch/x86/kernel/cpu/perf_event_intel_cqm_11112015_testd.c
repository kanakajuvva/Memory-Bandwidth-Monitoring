...
/*
 * Used to convert the bandwidth units from Bytes/sec to MB/sec
 * If MBM_CONVERSION_FACTOR is applied twice, then bandwidth units will be
 * MB/sec; this is because 1MB/sec  = 1000 * 1000 Bytes/sec.
 */
#define MBM_CONVERSION_FACTOR	1	//1000
 ...
 * @bytes:         current MSR value
 * @bandwidth:     memory bandwidth
 * @prev_time:     time stamp of previous sample
 */
struct sample {
	u64 bytes;
	u64 bandwidth;
 ...
 * If MSR is read within last 100ms, then then the sample is ignored;
 ...
 * cum_avg is the raw bandwidth in Bytes/sec.
 ...
 *
 */
static u64 rmid_read_mbm(unsigned int rmid, bool is_mbm_local)
{
	u64  val, currentmsr, diff_time, prevbw, bytes;
	...
	currentmsr = val;
	val &= MBM_CNTR_MAX;
	/* if MSR current read value is less than MSR previous read value
	 * then it is an overflow. MSR values are increasing when bandwidth
	 * consumption for the thread is non-zero; Overflow occurs, When
	 * MBM counter value reaches its maximum i.e. MBM_CNTR_MAX.
	 *
	 * After overflow, MSR current value goes back to zero and starts
	 * increasing again at the rate of bandwidth.
	 *
	 * Overflow handling:
	 * First overflow is detected by comparing current msr values will
	 * with the last read value. If current msr value is less than
	 * previous value then it is an overflow. When overflow occurs,
	 * (MBM_CONTR_MAX - prev msr value) is added the current msr value
	 * to the get actual value.
	 */
	...
	/*
	 * MBM_TIME_DELTA_EXP is picked as per MBM specs. As per hardware
	 * functionality, overflow can occur maximum once in a second. So
	 * latest we want to read the MSR counters is 1000ms. If it is less
	 * than 1000ms we can  ignore the sample. Then we decide since when
	 * we should ignore the samples. If the MSR was Read with in last
	 * 100ms, why process the MSR reads again. Anyway there'll be small
	 * change or zero change.
	 *
	 * So ignoring MSR Reads within 100ms is efficient.
	 * MBM_TIME_DELTA_MIN was defined as 100ms as per this guideline.
	 */

	val =  (val * MBM_TIME_DELTA_EXP) / diff_time;

	if ((diff_time > MBM_TIME_DELTA_EXP) && (!prevbw))
	...
	if ((diff_time <=  MBM_TIME_DELTA_EXP + MBM_TIME_DELTA_MIN) ||
	    overflow || first) {
		int currentbw, averagebw;

		/*
		 * For the first 'mbm_window_size -1' samples
		 * calculate average by adding the current sample's bandwidth
		 * to the sum of existing bandwidth values and dividing the sum
		 * with the number of samples profiled so far
		 */
		currentbw = val;
		averagebw = 0;
		/* convert the bandwidth to MB/sec */
		if (currentbw)
			currentbw = DIV_ROUND_UP(currentbw,
				    MBM_CONVERSION_FACTOR);

		mbm_current->bandwidth = currentbw;
...
static void intel_mbm_event_update(struct perf_event *event)
{
	...
	case QOS_MBM_TOTAL_EVENT_ID:
		val = rmid_read_mbm(rmid, false);
		break;
	case QOS_MBM_LOCAL_EVENT_ID:
		val = rmid_read_mbm(rmid, true);
		break;
	default:
		return;
	}
...
static u64 intel_mbm_event_count(struct perf_event *event, struct rmid_read *rr)
{
	...
	//rr->evt_type = event->attr.config >> 1;
...
static u64 intel_cqm_event_count(struct perf_event *event)
{
	...
	if (((event->attr.config == QOS_MBM_TOTAL_EVENT_ID) ||
		...
		rr.evt_type = event->attr.config & 0x01;
...
EVENT_ATTR_STR(llc_total_bw.runavg_nosamples, intel_cqm_llc_total_bw_runavg_nosamples, "10");
EVENT_ATTR_STR(llc_local_bw.runavg_nosamples, intel_cqm_llc_local_bw_runavg_nosamples, "10");
...
static struct attribute *intel_mbm_events_attr[] = {
	...
	EVENT_PTR(intel_cqm_llc_total_bw_runavg_nosamples),
	EVENT_PTR(intel_cqm_llc_local_bw_runavg_nosamples),
	NULL,
};
...
static int  intel_mbm_init(void)
{
	 ...
	 * rmid_read_mbm()
	 */
	mbm_scale_rounded =  DIV_ROUND_UP(cqm_l3_scale*1000, 1048); //MBM_CONVERSION_FACTOR);

	snprintf(scale, sizeof(scale), "%u%s", mbm_scale_rounded,"e-6");
