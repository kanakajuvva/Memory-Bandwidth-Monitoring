	...
	if (a->hw.target == b->hw.target) {
		if (a->attr.config  != b->attr.config ) {
			struct cqm_rmid_entry *entry;
				...
				pr_err("  rmid %u multi event %d \n",a->hw.cqm_rmid,entry->is_multi_event);
		}
		return true;
	}
...
static u64 rmid_read_mbm(unsigned int rmid, bool is_mbm_local)
{
	 ...
	 * latest we want to read the MSR counters is 1000ms. Minimum time
	 * interval between two MSR reads is 100ms. If read_rmid_mbm is
	 * called with in less than 100ms, use the previous sammple since
	 * perf also recommends to use the minimum sampling period of 100ms.
	 */

	val =  (val * MS_PER_SEC) / diff_time;
...
static u64 intel_cqm_event_count(struct perf_event *event)
{
	 ...
	 */
	pr_err(" count %u",event->attr.config);
	if (event->cpu != -1)
		return __perf_event_count(event);
	...
	if (!cqm_group_leader(event)){
		...
		if (entry->is_multi_event)
			pr_err(" is multi event %u %d ",event->hw.cqm_rmid,entry->is_multi_event);
		else
			return 0;
	}
...
EVENT_ATTR_STR(llc_total_bw.runavg_nosamples,
			intel_cqm_llc_total_bw_runavg_nosamples, "10");
EVENT_ATTR_STR(llc_local_bw.runavg_nosamples,
			intel_cqm_llc_local_bw_runavg_nosamples, "10");
...
static int  intel_mbm_init(void)
{
	 ...
	 * Bytes / 1.048e6 gives the MBytes.  Hardware uses upscale factor
	 * as given by cqm_l3_scale. Muliply upscale factor by 1/1.048e6
	 * to set the scale to get the perf output in MBytes/sec
	 */
	mbm_scale_rounded =  DIV_ROUND_UP(cqm_l3_scale * 1000, 1048);
